...
Modified by Logan Voegtly and Matthew Lueder:
...
def _get_qacct_exit_status(job_id):
    result = subprocess.run(['qacct', '-d', '5', '-j', str(job_id)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def _build_qsub_command(cmd, job_name, outfile, errfile, pe, n_cpu, queue):
    ...
    -r:  If the value of the option-argument is y, the qsub utility shall set the 'Rerunable' attribute of
         the batch job to TRUE.
    """

    qsub_template = """echo {cmd} | qsub -o ":{outfile}" -e ":{errfile}" -V -r y -q {queue} -pe {pe} {n_cpu} -N {job_name}"""
    return qsub_template.format(
        cmd=cmd, job_name=job_name, outfile=outfile, errfile=errfile,
        pe=pe, n_cpu=n_cpu, queue=queue)
...
class SGEJobTask(luigi.Task):
    ...
    sge_queue = luigi.Parameter(
        significant=False,
        description="Name of the queue to use on the SGE",
        default='all.q'
    )
    ...
    def _run_job(self):
        ...
        submit_cmd = _build_qsub_command(job_str, self.job_name, self.outfile,
                                         self.errfile, self.sge_parallel_env, self.n_cpu, self.sge_queue)
        ...
        logger.info("Submitted job to qsub with response:\n" + output)
    ...
    def _track_job(self):
        while True:
            ...
            exit_code = _get_qacct_exit_status(self.job_id)
            if exit_code == 0:
                logger.debug('SGE job for task {} completed without error.'.format(self.task_id))
                time.sleep(2)
                break
            elif exit_code > 0:
                raise RuntimeError('SGE job for task {} completed with error code {}.'.format(self.task_id, exit_code))
